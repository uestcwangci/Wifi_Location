...
################################################################################################
# 读入特征值训练集
filename = mat_path + '\\x_train_r5Loc.mat'
sample = sio.loadmat(filename)
sample = sample['x_train_r5Loc'].astype('float32')
# 特征向量归一化
sample /= 100.0
# sample = np.array(sample)
# 读入标签训练集
filename = mat_path + '\\y_train_r5Loc.mat'
label = sio.loadmat(filename)
label = label['y_train_r5Loc'] - 1
...
for hid_num in range(5, 30):   # 隐层节点数(经验公式) h=sqrt(m+n)+a m为输入结点数，n为输出结点数，a为1~10整数
    ##################################################################################################
    # 神经网络配置
    ...
    right_temp = []
    # 读入特征测试集
    filename = mat_path + '\\x_test_r5Loc.mat'  # raw_input() # 换成raw_input()可自由输入文件名
    test = sio.loadmat(filename)
    test_s = test['x_test_r5Loc'].astype('float32')
    # 特征向量归一化
    test_s /= 100.0
    # test_s = np.array(test_s)
    # 读入标签测试集
    filename = mat_path + '\\y_test_r5Loc.mat'  # raw_input() # 换成raw_input()可自由输入文件名
    test_label = sio.loadmat(filename)
    test_l = test_label['y_test_r5Loc'] - 1
    ...
    if 'w1_5loc.txt' == file:
        for iW in top_right['W1']:
            for iw in iW:
                ...
                output.write(' ')
            output.write('\n')
    elif 'w2_5loc.txt' == file:
        for iW in top_right['W2']:
            for iw in iW:
                output.write(str(iw))
                output.write(' ')
            output.write('\n')
    elif 'hid_offset_5loc.txt' == file:
        for iSet in top_right['Hid_off']:
            output.write(str(iSet))
            output.write('\n')
    elif 'out_offset_5loc.txt' == file:
...
# 输出结果
###################################################################################################
Network = open("NetWork_5loc.txt", 'w')
...
Network.close()
###################################################################################################
para_out('w1_5loc.txt')
para_out('w2_5loc.txt')
para_out('hid_offset_5loc.txt')
para_out('out_offset_5loc.txt')


# 输出分布结果
###################################################################################################
Accuracy = open('AccuracyRate_5loc.txt', 'w')
